...
export const CartProvider = ({ children }) => {
  ...
  const fetchCartItems = async () => {
    ...
    if (token) {
      try {
        const response = await axios.get('http://localhost:5000/api/cart/getCartItems', {
  ...
  const updateCartItemQuantity = async (productId, quantity) => {
    try {
      await axios.post("http://localhost:5000/api/cart/updateQuantity", { productId, quantity }, { withCredentials: true });
    } catch (error) {
